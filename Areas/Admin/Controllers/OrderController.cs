...
namespace hoangstore.Areas.Admin.Controllers
{
    ...
    public class OrderController : Controller
    {
        ...
        {
            var orderQuery = _db.Orders.AsNoTracking().AsQueryable();
            if (orderStatus.HasValue)
            {
                orderQuery = orderQuery.Where(o => o.Status == orderStatus.Value);
            }
        ...
        public async Task<IActionResult> UpdateStatus(int orderId, OrderStatus orderStatus)
        {
            ...
            // Neu chon thanh toan ttuyen
            if (order.PaymentMethod == "VNPAY")
            {
                //neu status = pending
                // 
                if (order.Status == OrderStatus.Pending && orderStatus != OrderStatus.Cancelled && orderStatus != OrderStatus.Pending)
                {
                    TempData["ErrorMessage"] = "Đơn hàng VNPAY này chưa được khách thanh toán tiền! Chỉ được phép Hủy hoặc chờ khách thanh toán.";
                    return RedirectToAction("Details", new { id = orderId });
                }
            }
